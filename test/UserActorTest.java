...
public class UserActorTest {
    ...
    public void testOnSearchVideos() throws JsonProcessingException {
        WSClient wsClient = mock(WSClient.class); // Mock the WSClient
        WSRequest wsRequest = mock(WSRequest.class); // Mock WSRequest
        WSResponse wsResponse = mock(WSResponse.class); // Mock WSResponse

        ObjectMapper mapper = new ObjectMapper(); // To parse JSON

        // Mocking the WSClient behavior
        ...
        assertEquals(2, response.getVideos().size()); // Validate video count

        // Validate individual video details
    ...
    public void testOnEmptyApiResponse() throws JsonProcessingException {
        ...
        assertEquals(SEARCH_KEYWORD, response.getQuery());
        assertEquals(0, response.getVideos().size()); // No videos should be returned
    }
